...
async function addDefaultUser() {
  try {
    ...
    const [roles] = await connection.execute('SELECT * FROM tbl_role WHERE role = ?', ['admin']);
    ...
    if (roles.length === 0) {
      ...
      const [newRoles] = await connection.execute('SELECT * FROM tbl_role WHERE role = ?', ['admin']);
      if (newRoles.length > 0) {
        adminRoleId = newRoles[0].id;
      }
    } else {
